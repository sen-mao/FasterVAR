...
def prepare_images(savedir_pred,savedir_gt,category):
    ...
    repo_id="/nfs-26/maxiaoxiao/ckpts/PixArt-XL-2-1024-MS"
    ...
    for item in batched_iterator_MJHQ(mjhq_root,batch_size=4,select_size=4,
        ...
        for i,label in enumerate(name):
            list_.append(osp.exists(osp.join(savedir_pred,category[i],'%s.png'%label)))
        if all(list_):
...
if __name__=="__main__":
    save_root='/nfs-141/maxiaoxiao/eval_results/compare_methods_local/FID_mjhq_1024/'
    savedir_pred=osp.join(save_root,'prediction_pixart_alpha')
