...
def prepare_images(savedir_pred,savedir_gt):
    ...
    for item in batched_iterator_MJHQ(mjhq_root,batch_size=8,select_size=select_sz,
                                      category=["food","indoor","landscape","logo","people","plants","vehicles"]):
        ...
        if not len(text_prompts_selected)==0:
            oup_B3HW=pipe(text_prompts_selected,height=512,width=512).images
