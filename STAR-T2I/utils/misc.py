...
def auto_resume_vae(args: arg_util.Args, pattern='ckpt*.pth') -> Tuple[List[str], int, int, dict]:
    info = []
    ...
    if len(all_ckpt) == 0:
        ...
        return info, ep, it, ckpt['vqvae']


def auto_resume(args: arg_util.Args, pattern='ckpt*.pth') -> Tuple[List[str], int, int, dict]:
    info = []
    if args.pretrained_ckpt == "" or args.pretrained_ckpt is None:
        file = os.path.join(args.local_out_dir_path, pattern)
        all_ckpt = glob_with_latest_modified_first(file)
    elif os.path.exists(args.pretrained_ckpt):
        all_ckpt = [args.pretrained_ckpt]
    else:
        raise Exception(f"Not found {args.pretrained_ckpt}")
    if len(all_ckpt) == 0:
        info.append(f'[auto_resume] no ckpt found @ {file}')
        info.append(f'[auto_resume quit]')
        return info, 0, 0, {}
        ...
        ep, it = ckpt['epoch'], ckpt['iter']
        info.append(f'[auto_resume success] resume from ep{ep}, it{it}')
        return info, ep, it, ckpt['trainer']