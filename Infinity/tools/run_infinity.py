...
def encode_prompt(text_tokenizer, text_encoder, prompt, enable_positive_prompt=False):
    ...
    text_features = text_encoder(input_ids=input_ids, attention_mask=mask)['last_hidden_state'].float()  # (1, 512, 2048)
    lens: List[int] = mask.sum(dim=-1).tolist()
    cu_seqlens_k = F.pad(mask.sum(dim=-1).to(dtype=torch.int32).cumsum_(0), (1, 0))
    Ltext = max(lens)    
    kv_compact = []
    for len_i, feat_i in zip(lens, text_features.unbind(0)):
        kv_compact.append(feat_i[:len_i])  # text_features (len(captions[0]), 2048)
