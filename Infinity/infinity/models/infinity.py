...
from infinity.utils.rank_ratio import rank_ratio
...
from utils.low_rank_feature import representative_token_indices, random_projection_for_low_rank_feature, \
    low_rank_approximation_percent
...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        kv_compact_un = kv_compact[le:]
        ca_kv_un = kv_compact_un, cu_seqlens_k[:-1], max_seqlen_k
        ...
        if inference_mode:
            for b in self.unregistered_blocks: (b.sa if isinstance(b, CrossAttnBlock) else b.attn).kv_caching(True)  ## only self attention using kv_caching
        else:
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            for block_idx, b in enumerate(self.block_chunks):
                ...
                ## Low-rank feature
                low_rank_scales = (40, 48, 64)  # (40,48,64)
                if pn[-1] in low_rank_scales:
                    ## Predetermination Strategy.
                    # _, low_rank_ratio = low_rank_approximation_percent(last_stage, alpha)
                    ## Predetermination Rank.
                    rank = int(rank_ratio[alpha][f'scale{pn[-1]}'][block_idx] * last_stage.shape[1]) + 1
                    ## Representative Token Indices Selection with probability Pi.
                    rep_token_indices = representative_token_indices(last_stage, p=rank)
                    ## Random Projection (RP) for Low-Rank Feature.
                    last_stage = random_projection_for_low_rank_feature(last_stage, r=rank)

                for m in b.module:
                    if cfg == 0 and m.sa.cached_k is not None and m.sa.cached_k.shape[0] > 1:
                        m.sa.cached_k = m.sa.cached_k[B:]
                        m.sa.cached_v = m.sa.cached_v[B:]
                    last_stage = m(x=last_stage, cond_BD=cond_BD_or_gss if cfg>0 else cond_BD_or_gss[B:], ca_kv=ca_kv if cfg>0 else ca_kv_un, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si)
                ...
                if pn[-1] in low_rank_scales:
                    def unmerge(unmerged_cur_x, unmerged_cache_x, cached_hw=None):
                        ...
                        if cfg == 0 and unmerged_cache_x.shape[0] > 1:
                            unmerged_cache_x = unmerged_cache_x[B]
                        unmerged_cache_x_ = unmerged_cache_x.view(B_, cached_hw[-1], cached_hw[-1], -1).permute(0, 3, 1, 2)
                        unmerged_cache_x_ = torch.nn.functional.interpolate(unmerged_cache_x_, size=(pn[-1], pn[-1]), mode='area').permute(0, 2, 3, 1).view(B_, pn[-1]**2, d)
                        unmerged_cache_x_.scatter_(dim=1, index=rep_token_indices[:,:,None].repeat(1, 1, d), src=unmerged_cur_x)
                        return unmerged_cache_x_
                    last_stage = unmerge(last_stage, self.last_stages[block_idx], cached_hw=self.cached_hw)

                last_stages += [last_stage]

            self.last_stages = last_stages
            self.cached_hw = pn

            if cfg == 0:  ## Semantic irrelevance
                logits_BlV = self.get_logits(last_stage, cond_BD[B:]).mul(1/tau_list[si])
            elif (cfg != 1) and add_cfg_on_logits:
