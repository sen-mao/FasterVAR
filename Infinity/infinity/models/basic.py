...
class FFN(nn.Module):
    ...
    def forward(self, x):
        if self.fused_mlp_func is not None:
            ...
            # # TODO: visualization ffn
            # from utils.ffn import plot_ffn_features
            # plot_ffn_features(self.drop(self.fc2( self.act(self.fc1(x.clone())))), layer_idx=self.layer_idx)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):
        ...
        # # TODO: visualization self-attention
        # from utils.self_attention_map import plot_attention_weights
        # plot_attention_weights(query=q, key=k, value=v, scale=None, attn_mask=attn_bias_or_two_vector, dropout_p=0, layer_idx=self.layer_idx)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv):
        ...
        # # TODO: visualization cross-attention
        # if hasattr(self, 'layer_idx'):
        #     from utils.cross_attention_map import plot_attention_weights
        #     plot_attention_weights(query=q_compact, key=kv_compact[:,0], value=kv_compact[:,1], scale=None, oup=oup, layer_idx=self.layer_idx)
...
class CrossAttnBlock(nn.Module):
    ...
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_norm_func is None:
            ...
            # # TODO: 1. low-rank for low_rank_feature
            # low_rank_scales = (32,)
            # scale_x = int(x.shape[1] ** 0.5)
            # if scale_x in low_rank_scales:  # (20, 24, 32, 40, 48, 64):
            # #     from utils.low_rank_feature import low_rank_approximation_percent
            # #     x = low_rank_approximation_percent(x, sv_percent=0.99)
            #     from utils.low_rank_feature import low_rank_prune_percent
            #     rank_percent = 0.99 if scale_x in (40, 48,) else 0.99999
            #     x, U = low_rank_prune_percent(x, rank_percent=rank_percent)
            ...
            # # TODO: 1. unmerge sa token
            # if scale_x in low_rank_scales:  # (20, 24, 32, 40, 48, 64):
            #     x = U @ x

            x = x + self.drop_path(x_sa.mul_(gamma1))

            # # TODO: 2. low-rank for low_rank_feature
            # low_rank_scales = (32,)
            # scale_x = int(x.shape[1] ** 0.5)
            # if scale_x in low_rank_scales:  # (20, 24, 32, 40, 48, 64):
            #     # from utils.low_rank_feature import low_rank_approximation_percent
            ...
            # # TODO: 2. unmerge ca token
            # if scale_x in low_rank_scales:  # (20, 24, 32, 40, 48, 64):
            #     x = U @ x

            # # TODO: 3. low-rank for low_rank_feature
            # low_rank_scales = (32,)
            # scale_x = int(x.shape[1] ** 0.5)
            # if int(x.shape[1] ** 0.5) in low_rank_scales:
            #     # from utils.low_rank_feature import low_rank_approximation_percent
            #     # x = low_rank_approximation_percent(x, sv_percent=0.99)
            #     from utils.low_rank_feature import low_rank_prune_percent
            #     rank_percent = 0.99 if scale_x in (40, 48,) else 0.99999
            ...
            # # TODO: 3. unmerge ffn token
            # if scale_x in low_rank_scales:
            #     x = U @ x
        return x
