...
def low_rank_approximation_percent(last_stage, alpha):
    """
    对三维矩阵 last_stage 做逐样本低秩近似（基于 torch.pca_lowrank）

    输入:
      last_stage: torch.Tensor, shape=(B, m, n)
      alpha: float, threshold

    输出:
      last_stage_approx: torch.Tensor, shape=(B, m, n)
    """
    assert last_stage.dim() == 3, "输入必须是三维 (B, m, n)"

    B, m, n = last_stage.shape
    q = min(m, n)
    ...
    with torch.cuda.amp.autocast(enabled=False):
        ...
        last_stage_approx = torch.bmm(U * S.unsqueeze(1), V.transpose(1, 2)) + last_stage_mean
        low_rank_ratio = (rank_needed.to(torch.float32) / q).tolist()

    return last_stage_approx, low_rank_ratio

def random_projection_for_low_rank_feature(last_stage: torch.Tensor, r: int = 128):
    ...
    """
    b, H, W = last_stage.shape
    token_type = last_stage.dtype

    with torch.cuda.amp.autocast(enabled=False):
        last_stage = last_stage.to(torch.float32)
        Q = torch.randn(b, H, r, device=last_stage.device) * (1.0 / math.sqrt(r))
        projected = torch.bmm(Q.transpose(1, 2), last_stage)  # (B, r, W)

    return projected.to(token_type)

def representative_token_indices(last_stage: torch.Tensor, p: int, c: float = 0.1, replace: bool = False):
    """按论文方案对 last_stage 做行采样 (支持 batch 维度，torch 实现)

    Args:
        last_stage      : (B, m, n) 的张量
        p      : 每个 batch 采样的行数
        c      : 常数 c<=1，控制概率下限
        replace: 是否放回采样
