...
def main(args):
    ...
    model.diffloss = model.diffloss.float()
    model.eval()

    if args.use_ema:
        ema_state_dict = torch.load(
            os.path.join(args.model_path, "ema_model.bin"), map_location="cpu"
        )
    ...
    scale = (1, 2, 3, 4, 5, 7, 9, 12, 16, 21, 27, 36, 48, 64)
    ...
    with torch.inference_mode():
        for _ in range(10):
            ...
            with torch.autocast(
                ...
                infer_func = (
                    ema_model.autoregressive_infer_cfg_fastervar
                    if args.use_ema
                    else model.autoregressive_infer_cfg_fastervar
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", type=str, help="", default="")
