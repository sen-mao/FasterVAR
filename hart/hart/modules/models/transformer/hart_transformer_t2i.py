...
from hart.utils.rank_ratio import rank_ratio
from hart.utils.low_rank_feature import low_rank_approximation_percent, \
    random_projection_for_low_rank_feature, \
    representative_token_indices

device = torch.device("cuda")

def mask_by_order(mask_len, order, bsz, seq_len):
    masking = torch.zeros(bsz, seq_len).to(device)  #.cuda()
    masking = torch.scatter(
        masking,
        dim=-1,
        index=order[:, : mask_len.long()],
        src=torch.ones(bsz, seq_len).to(device),  #.cuda(),
    ).bool()
    return masking
...
class HARTForT2I(PreTrainedModel):
    ...
    def __init__(self, config: HARTForT2IConfig):
        ...
        vae_local = vae_local.to(device)  #.cuda()
    ...
    @torch.no_grad()
    def autoregressive_infer_cfg_fastervar(  # CFG=0
            self,
            B: int,
        ...
        sos = cond_BD = self.context_embed(
            self.context_norm(
                torch.cat((label_B, torch.full_like(label_B, fill_value=0.0)), dim=0)  # TODO
            )
        )
        ...
        for si, pn in enumerate(self.patch_nums[:-1]):  # si: i-th segment
            ...
            for i, b in enumerate(self.blocks):
                if 0 == cfg[si] and b.attn.cached_k is not None and b.attn.cached_k.shape[0] > 1:
                    b.attn.cached_k = b.attn.cached_k[B:]
                    b.attn.cached_v = b.attn.cached_v[B:]
                # Haotian: si used for position embed
                x = b(
                    x=x,
                    cond_BD=cond_BD_or_gss if cfg[si]>0 else cond_BD_or_gss[B:],
                    attn_bias=None,
                    si=si,
                    context_position_ids=context_position_ids if cfg[si]>0 else context_position_ids[B:],
                    context_mask=context_mask if cfg[si]>0 else context_mask[B:],
                )
                x_blocks += [x]
            ...
            logits_BlV = self.get_logits(x, cond_BD if cfg[si]>0 else cond_BD[B:])
            ...
            f_hat, next_token_map, h = self.vae_quant_proxy[
                0
            ].get_next_autoregressive_input_(
        ...
        mask = torch.ones(B, self.last_level_pns).to(device)  #.cuda()
        tokens = torch.zeros(B, self.last_level_pns, self.Cvae).to(device)  #.cuda()
        ...
        for step in indices:
            ...
            mask_len = torch.Tensor([np.floor(self.last_level_pns * mask_ratio)]).to(device)  #.cuda()
            ...
            for i, b in enumerate(self.blocks):
                low_rank_scales = (64,)
                rep_token_indices = torch.arange(4096, dtype=torch.int32).unsqueeze(0).expand(2, -1)
                rank = 4096
                if pn in low_rank_scales:
                    ## Predetermination Strategy.
                    # _, low_rank_ratio = low_rank_approximation_percent(x, alpha=float(alpha))
                ...
                x = b(
                    ...
                    attn_bias=None,
                    si=len(self.patch_nums) - 1,
                    m_maskgit=(torch.concat((cur_mask[0][:4096][:rank],cur_mask[0][4096:][:rank])), torch.concat((cur_mask[1][:4096][:rank],cur_mask[1][4096:][:rank]))) if cfg[si]>0 else (cur_mask[0][:4096][rep_token_indices[0]], cur_mask[1][:4096][rep_token_indices[0]]),
                    context_position_ids=context_position_ids if cfg[si]>0 else context_position_ids[B:],
                    context_mask=context_mask if cfg[si]>0 else context_mask[B:],
                )
                ...
                if pn in low_rank_scales:
                    def unmerge(unmerged_cur_x, unmerged_cache_x, cached_hw=None):
                        ...
                        unmerged_cache_x_.scatter_(
                            dim=1,
                            index=rep_token_indices[:, :, None].repeat(1, 1, d),
                            src=unmerged_cur_x,
                        )
            ...
            if final_stage == 0:
                ...
                with torch.amp.autocast("cuda", enabled=False):
                    h_BChw_diff = self.diffloss.sample(
                        z=last_stage_cond.float(), temperature=1.0, cfg=float(t)
                    )
    ...
    def sample_orders(self, bsz):
        ...
        for _ in range(bsz):
            ...
            orders.append(order)
        orders = torch.Tensor(np.array(orders)).to(device).long()  #.cuda().long()
        return orders
